...
class WebSocketManager:
    ...
    def __init__(self):
        """
        self ref: https://medium.com/@nandagopal05/scaling-websockets-with-pub-sub-using-python-redis-fastapi-b16392ffe291
    ...
    async def broadcast_to_room_from_restapi(self, room_id: str, message: str) -> None:
        await self.pubsub_client.connect()
        pubsub_subscriber = await self.pubsub_client.subscribe(room_id)
        asyncio.create_task(self._pubsub_data_reader(pubsub_subscriber))
        await self.pubsub_client._publish(room_id, message)